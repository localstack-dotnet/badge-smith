...
public sealed class TestResultsBadgeHandlerTests
{
    ...
    [Fact]
    public async Task HandleAsync_Should_Pass_Parameters_In_Service_Owner_Repo_Platform_Branch_Order_When_Badge_Is_Requested()
    {
        ...
        var sut = new TestResultsBadgeHandler(Mock.Of<ILogger<TestResultsBadgeHandler>>(), testResultsService.Object);

        var response = await sut.HandleAsync(CreateRouteContext(), TestContext.Current.CancellationToken);

        Assert.Equal(200, response.StatusCode);
        Assert.Contains(""""label":"tests"""", response.Body, StringComparison.Ordinal);
    ...
    {
        var sut = new TestResultsBadgeHandler(
            Mock.Of<ILogger<TestResultsBadgeHandler>>(),
    ...
    private static TestResultEntity CreateEntity() => new(
        ...
        Commit: "abc1234",
        RunId: "42",
        UrlHtml: "https://example.com/run/42",
