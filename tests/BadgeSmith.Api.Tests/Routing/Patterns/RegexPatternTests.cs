...
public sealed class RegexPatternTests
{
    ...
    public void TryMatch_Should_HandleComplexPackageNames()
    {
        ...
        {
            "/badges/packages/nuget/Microsoft.Extensions.DependencyInjection", "/badges/packages/nuget/Microsoft.Extensions.DependencyInjection.Abstractions",
            "/badges/packages/nuget/System.Text.Json", "/badges/packages/github/some.complex.package-name",
    ...
    public static IEnumerable<object[]> GetSourceGeneratedRegexData()
    {
        ...
        [
            ...
            new Func<Regex>(() =>
                new Regex(@"^/badges/tests/(?<platform>\w+)/(?<owner>[\w-]+)/(?<repo>[\w.-]+)/(?<branch>[\w.-]+)$", RegexOptions.IgnoreCase | RegexOptions.Compiled)),
